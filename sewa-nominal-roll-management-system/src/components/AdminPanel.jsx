...
export default function AdminPanel({ session, profile, onProfileUpdate, showToast }) {
    ...
    <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>
          Manage who can edit the nominal roll. Users with edit access can add rows, edit cells, and upload PDFs.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--accent-600)' }}>{users.filter(u => u.role === 'admin').length}</p>
        </div>
        <div style={{
          background: 'white',
          padding: 16,
          ...
          <p style={{ fontSize: 24, fontWeight: 700, color: 'var(--warning-600)' }}>{users.filter(u => u.can_edit).length}</p>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 600, fontSize: 12, color: 'var(--neutral-600)', background: 'var(--neutral-50)', borderBottom: '2px solid var(--border)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>Edit Access</th>
              </tr>
            </thead>
            <tbody>
              {users.map(u => (
      ...
      <p style={{ marginTop: 16, fontSize: 12, color: 'var(--text-muted)', textAlign: 'center' }}>
        Toggle edit access to allow users to add rows, edit cells, and upload PDFs. Admins always have full access.
      </p>
    </div>
  )
}
